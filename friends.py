...
class FriendsDataset(Dataset):
    def __init__(self, context_length: int, texts: Scripts, vocab: Vocab, train: bool) -> None:
        super().__init__()
        self.context_length = context_length
        self.train = train
        self.data = torch.tensor(vocab.encode(texts.content), dtype=torch.long)
...
class FeedForward(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # amplify signal cf paper llama 2 
...
if __name__ == '__main__':
    ...
    texts = Scripts('friends_transcript.txt')
    vocab = Vocab()
    
    train_set = FriendsDataset(context_length, texts, vocab, train=True)
    val_set = FriendsDataset(context_length, texts, vocab, train=False)
    ...
    # optimizer = bnb.optim.AdamW8bit(optim_groups, lr=lr, betas=(0.9, 0.99))
    ...
    torch.save(model.state_dict(), 'friends.pt')
    torch.onnx.export(
        model.cpu(),
    ...
    # quantize_dynamic('friends.onnx', 'friends.8bit.onnx')